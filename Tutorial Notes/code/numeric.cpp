...
double sum_every_other_element(const std::vector<double>& vec)
{
    ...
    // TODO: Since we'll hit half of vec, make a new vector with half the size of type unsigned int
    ...
    std::for_each(indices.begin(), indices.end(), [](unsigned int & val){val = val * 2;});
    ...
    double sum = 0.0;
    sum = std::accumulate(indices.begin(), indices.end(), 0.0, [&](double sum, unsigned int val) {return sum += val;});
    return sum;
}
